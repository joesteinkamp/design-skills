...
import type { SkillFrontmatter } from "../src/skills/schema.js";

const minimalSkill: SkillFrontmatter = {
  name: "test",
  description: "test",
  tags: [],
  upstream_skills: [],
...
describe("collectInputs (non-interactive)", () => {
  it("maps --<input> flags to skill inputs", async () => {
    const out = await collectInputs(minimalSkill, {
      flags: {
  ...
  it("throws on missing required input", async () => {
    await expect(
      collectInputs(minimalSkill, { flags: {}, interactive: false }),
    ).rejects.toThrow(/feature_description/);
  });

  it("throws on missing required user_input", async () => {
    await expect(
      collectInputs(minimalSkill, {
        flags: { feature_description: "x" },
        interactive: false,
      }),
